...
def quat_geodesic_loss_rad(q_pred: torch.Tensor, q_true: torch.Tensor, eps: float = 1e-8) -> torch.Tensor:
    """Geodesic loss between quaternions in **radians** (squared angle).

    q_pred, q_true: [B,4] in (qw,qx,qy,qz) order.
    Handles the q and -q equivalence by using |dot|.
    """
    q_pred = q_pred / (q_pred.norm(dim=1, keepdim=True).clamp_min(eps))
    q_true = q_true / (q_true.norm(dim=1, keepdim=True).clamp_min(eps))

    dot = (q_pred * q_true).sum(dim=1).abs().clamp(min=-1.0 + 1e-7, max=1.0 - 1e-7)
    ang = 2.0 * torch.acos(dot)  # [0, pi]
    return (ang * ang).mean()
...
def train_multitask(
    ...
    # Pre-build per-task de-normalization tensors for pose (x)
    # NOTE: x is pose for IK and joints for FK; we only use these for IK aux losses.
    pose_denorm: Dict[str, Tuple[torch.Tensor, torch.Tensor]] = {}
    for tn in task_names:
        xm = torch.tensor(task_norm[tn]["x_mean"][0], dtype=torch.float32, device=device).view(1, 7)
        xs = torch.tensor(task_norm[tn]["x_std"][0], dtype=torch.float32, device=device).view(1, 7)
        pose_denorm[tn] = (xm, xs)
    ...
    for gstep in pbar:
        ...
        if mode == "fk":
            ...
            q_pred, pos_pred_norm, ori_pred_norm = model(x, mask)
            loss_q = masked_mse(q_pred, y, mask)

            # Denormalize pose targets (meters + unit quaternion) for a physically-meaningful aux loss.
            xm, xs = pose_denorm[tname]
            x_raw = x * xs + xm
            pos_t_raw = x_raw[:, :3]
            ori_t_raw = x_raw[:, 3:7]

            # Convert predicted aux outputs into raw space.
            pos_pred_raw = pos_pred_norm * xs[:, :3] + xm[:, :3]
            ori_pred_raw = ori_pred_norm * xs[:, 3:7] + xm[:, 3:7]

            loss_pos = mse(pos_pred_raw, pos_t_raw)
            loss_ori = quat_geodesic_loss_rad(ori_pred_raw, ori_t_raw)
            loss = loss_q + pos_weight * loss_pos + ori_weight * loss_ori

            # Extra logging for debugging the balance
            writer.add_scalar(f"train/{tname}_loss_q", loss_q.item(), gstep)
            writer.add_scalar(f"train/{tname}_loss_pos_raw", loss_pos.item(), gstep)
            writer.add_scalar(f"train/{tname}_loss_ori_rad2", loss_ori.item(), gstep)
            ...
            with torch.no_grad():
                ...
                for tn in task_names:
                    ...
                    if mode == "fk":
                        ...
                        qp, pp_norm, op_norm = model(xb, mb)
                        lq_t = masked_mse(qp, yb, mb)

                        xm_t, xs_t = pose_denorm[tn]
                        xb_raw = xb * xs_t + xm_t
                        pos_t_raw = xb_raw[:, :3]
                        ori_t_raw = xb_raw[:, 3:7]

                        pp_raw = pp_norm * xs_t[:, :3] + xm_t[:, :3]
                        op_raw = op_norm * xs_t[:, 3:7] + xm_t[:, 3:7]

                        lp_t = mse(pp_raw, pos_t_raw)
                        lo_t = quat_geodesic_loss_rad(op_raw, ori_t_raw)

                        l = (lq_t + pos_weight * lp_t + ori_weight * lo_t).item()
                    losses.append(l)
                    ...
                    torch.save(
                        {
                            ...
                            "pos_loss_weight": pos_weight,
                            "ori_loss_weight": ori_weight,
                            "ori_loss_type": "quat_geodesic_rad2",
                            "task_norm": task_norm,
                            "step": gstep + 1,
                            "total_steps": total_steps_for_schedule,
...
def parse_args():
    ...
    # Backwards compatible overall aux weight (old behavior: loss_q + aux*(pos+ori)).
    # New behavior (recommended): loss_q + w_pos*pos_raw_mse + w_ori*ori_geodesic_rad2
    p.add_argument("--aux_loss_weight", type=float, default=0.03,
                   help="(legacy) overall aux weight. If --pos_loss_weight/--ori_loss_weight not set, both default to this.")
    p.add_argument("--pos_loss_weight", type=float, default=None,
                   help="Weight for position aux loss (raw meters MSE). Default: aux_loss_weight")
    p.add_argument("--ori_loss_weight", type=float, default=None,
                   help="Weight for orientation aux loss (quaternion geodesic, radians^2). Default: aux_loss_weight")
...
def main():
    ...
    # Resolve aux weights
    pos_w = float(args.aux_loss_weight if args.pos_loss_weight is None else args.pos_loss_weight)
    ori_w = float(args.aux_loss_weight if args.ori_loss_weight is None else args.ori_loss_weight)
    print(f"[INFO] Loss weights: joint=1.0 | pos_w={pos_w} | ori_w={ori_w} (ori uses geodesic rad^2)")
