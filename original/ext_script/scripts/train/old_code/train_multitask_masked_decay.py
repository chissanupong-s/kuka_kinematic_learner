...
- supports LR warmup + LR decay (step or cosine) to avoid plateauing
"""
...
def load_dataset_tensor(path: str) -> torch.Tensor:
    """Loads:
      - directory of shards (.pt/.bin) each containing Tensor [N,14] or dict {"data": Tensor}
      - single .pt/.bin
      - CSV with columns [q1..q7, x,y,z,qw,qx,qy,qz]
    Returns Tensor [N,14] float32 on CPU.
    """
...
def compute_lr(
    ...
    """Return the LR for this global step (0-indexed)."""
    # Warmup first (linear from 0 -> base_lr)
    ...
    if schedule == "cosine":
        # cosine from base_lr -> lr_min
        t = min(max(after / denom, 0.0), 1.0)
        cos = 0.5 * (1.0 + math.cos(math.pi * t))
        return lr_min + (base_lr - lr_min) * cos

    if schedule == "step":
        # piecewise constant: base_lr * gamma^{#milestones passed}
        k = 0
...
def train_multitask(
    ...
    print(f"[INFO] LR schedule: {lr_schedule} | base_lr={lr} | lr_min={lr_min} | warmup_steps={warmup_steps}")
    if lr_schedule == "step":
        print(f"[INFO] Step milestones: {step_milestones} | gamma={step_gamma}")
    ...
    for it in pbar:
        # Update LR (warmup + decay) before this step
        cur_lr = compute_lr(
            base_lr=lr,
            lr_min=lr_min,
            step=it,
        ...
        if log_lr_every and (it + 1) % log_lr_every == 0:
            writer.add_scalar("train/lr", cur_lr, it)

        if (it + 1) % max(1, print_every) == 0:
            pbar.set_postfix(task=tname, loss=f"{loss.item():.4f}", lr=f"{cur_lr:.2e}")
...
def parse_args():
    ...
    p.add_argument("--lr_schedule", choices=["none", "step", "cosine"], default="cosine",
                   help="LR schedule applied per iteration.")
    p.add_argument("--lr_min", type=float, default=1e-5, help="Minimum LR for step/cosine schedules.")
    p.add_argument("--warmup_steps", type=int, default=2000, help="Linear warmup steps (0 disables).")
    p.add_argument("--step_milestones", type=str, default="100000,200000,400000",
                   help="For --lr_schedule step: comma-separated step indices.")
    p.add_argument("--step_gamma", type=float, default=0.5, help="For --lr_schedule step: multiply LR by gamma at each milestone.")
    p.add_argument("--log_lr_every", type=int, default=50, help="Log LR to tensorboard every N steps (0 disables).")
...
def main():
    ...
    task_norm = {"5dof": stats_dict_from_dataset(ds5), "6dof": stats_dict_from_dataset(ds6), "7dof": stats_dict_from_dataset(ds7)}
