...
\caption{Property-level comparison with related learning-based models on serial manipulators. \emph{Setting} summarises what each work targets and on which robot(s); \emph{DoF-vary} indicates whether the active DoF count is varied within a single robot.}
\label{tab:propcomp}
\centering
\footnotesize
\begin{tabular}{lccc}
\toprule
Method & Setting & DoF-vary & SE(3)-aware \\
\midrule
\textbf{This work} & kinematics on iiwa\,14 & \textbf{Yes} & No \\
KineNN~\cite{diprasetya2025} & kinematics on UR5 & No & Yes \\
K{\"o}ker~\cite{koker2004} & kinematics on 3-joint & No & No \\
Devin~\cite{devin2017} & policies, multi-robot & No & No \\
Chen~\cite{chen2018} & policies, multi-robot & No & No \\
Ghadirzadeh~\cite{ghadirzadeh2021} & policies, multi-robot & No & No \\
SE3-Net~\cite{byravan2017} & pose-from-RGBD & No & Yes \\
SE3-Pose-Net~\cite{byravan2018} & pose-from-RGBD & No & Yes \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Comparison to prior learning-based kinematic models.} The practical scenario we address is a single physical robot operated under \emph{multiple DoF configurations}: a distal joint locked for a constrained task, a joint disabled due to fault, or a redundant joint reserved for a secondary objective. Table~\ref{tab:propcomp} maps the cited prior works onto the relevant axes. Existing learning-based kinematic models such as KineNN~\cite{diprasetya2025} and K{\"o}ker~\cite{koker2004} assume a \emph{fixed DoF count} and would have to be retrained from scratch for each new active-joint convention; cross-platform meta-learning works~\cite{devin2017,chen2018,ghadirzadeh2021} vary the \emph{robot itself} and target control policies rather than kinematics; SE3-Net family methods~\cite{byravan2017,byravan2018} predict pose from sensor inputs and target a different input modality entirely. Our work is therefore \emph{complementary to, not competing with}, those efforts: it fills a specific gap (within-robot DoF-vary FK) that none of them addresses, while remaining lightweight (a single 17\,M-parameter network) and able to reuse the same trained model file under any active-joint convention via the mask channel. On the operational side, the three-stage pipeline reduces wall-clock per-DoF training cost by 80.5\%/92.7\%/99.5\%, and Fig.~\ref{fig:dataeff} shows that this compute saving is paired with a corresponding sample saving for low-DoF configurations: a property that follows directly from the shared representation and is not available to single-task or cross-platform baselines.
