...
\caption{Meta-kinematics ResMLP architecture and three-stage pipeline. (a)~Shared backbone with mask projection (Eq.~\ref{eq:mask_proj}) and a 7-D position+quaternion output head; the same backbone is reused at every stage. (b)~Stages: per-DoF single-task initialisation, joint multi-task training on $\mathcal{D}_5\!\cup\!\mathcal{D}_6\!\cup\!\mathcal{D}_7$, and short per-DoF adaptation.}
\label{fig:arch}
\end{figure}
