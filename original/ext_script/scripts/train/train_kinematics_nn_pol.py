...
def get_args():
    ...
    parser.add_argument(
        "--log_dir", type=str, default="runs/kinematics_pose",
...
def evaluate_on_loader(model, loader, device, criterion, aux_weight, mode: str):
    """Compute total and component losses on a given DataLoader."""
    model.eval()
    total_loss = 0.0
    total_loss_q = 0.0
    total_loss_pos = 0.0
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_idx, (x, y) in enumerate(train_loader):
            ...
            if args.mode == "fk":
                ...
                loss_q = loss  # for logging consistency
                loss_pos = torch.tensor(0.0, device=device)
                loss_ori = torch.tensor(0.0, device=device)
            else:  # ik with dual-head
                q_pred, pos_pred, ori_pred = model(x)

                # main joint loss (normalized)
                loss_q = criterion(q_pred, y)

                # aux pose reconstruction losses (normalized input space)
                pos_target = x[:, :3]      # normalized x,y,z
                ori_target = x[:, 3:7]     # normalized qw,qx,qy,qz
                loss_pos = criterion(pos_pred, pos_target)
                loss_ori = criterion(ori_pred, ori_target)
        ...
        # scheduler step on total val loss
        ...
        # update progress bar postfix with final epoch metrics
        global_bar.set_postfix(
            epoch=f"{epoch}/{args.epochs}",
