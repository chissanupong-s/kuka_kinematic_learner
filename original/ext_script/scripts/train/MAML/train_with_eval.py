...
def get_args():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "(Optional, IK mode) Path to a trained FK checkpoint used to "
            "compute IK end-effector position error in metres."
        ),
    )
    parser.add_argument(
        "--weight_decay",
...
def load_dataset_tensor(path: str) -> torch.Tensor:
    ...
    if os.path.isdir(path):
        ...
        for f_path in shard_files:
            print(f"[INFO] Loading shard: {f_path}")
            ...
            if isinstance(arr, torch.Tensor):
                if arr.ndim != 2 or arr.shape[1] != 14:
                    raise ValueError(
                        f"Shard {f_path} must be [N,14], got shape {tuple(arr.shape)}"
                    )
                arr = arr.cpu().float().numpy()
            else:
                raise TypeError(
                    f"Shard {f_path} must contain a Tensor or dict with 'data' key."
                )
            arrs.append(arr)
            total_rows += arr.shape[0]

        print(f"[INFO] Concatenating {len(arrs)} shards with total rows={total_rows}")
        full_np = np.concatenate(arrs, axis=0)
        if full_np.shape[1] != 14:
            raise ValueError(
                f"Concatenated array must have 14 columns, got shape {full_np.shape}"
            )
        ...
        if not isinstance(full_tensor, torch.Tensor):
            raise TypeError(f"File {path} must contain a Tensor or dict with 'data'.")
        if full_tensor.ndim != 2 or full_tensor.shape[1] != 14:
            raise ValueError(
                f"Expected tensor of shape [N,14], got {tuple(full_tensor.shape)}"
            )
        full_tensor = full_tensor.float()
    else:
        raise ValueError(
            f"Unsupported file type for {path}. Expect directory, .csv, .pt, or .bin."
        )
...
def eval_fk_pos_rmse_m(
    ...
    """Evaluate FK model's end-effector position error in metres on a loader.

    Assumes:
      - mode == 'fk'
      - loader yields (x, y) where y is *normalised* pose [x,y,z,qw,qx,qy,qz]
      - target_mean/target_std are the normalisation stats used for pose
        (full_dataset.target_mean/std when mode='fk').

    Returns:
      Scalar RMSE of end-effector position in metres.
    """
    model.eval()
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            pose_pred_norm = model(x)           # [B,7] normalised pose
            pose_pred = pose_pred_norm * tgt_std + tgt_mean
            pose_true = y * tgt_std + tgt_mean

            pos_pred = pose_pred[:, :3]         # [B,3]
            pos_true = pose_true[:, :3]         # [B,3]

            dist = torch.norm(pos_pred - pos_true, dim=1)  # [B]
            sum_sq_dist += torch.sum(dist ** 2).item()
            n_samples += dist.size(0)

    if n_samples == 0:
        return float("nan")

    mse = sum_sq_dist / n_samples
    rmse = math.sqrt(mse)
    return rmse
...
def eval_ik_joint_rmse_deg(
    ...
    with torch.no_grad():
        for x, y in loader:
            x = x.to(device, non_blocking=True)
            y = y.to(device, non_blocking=True)  # normalised joints

            q_pred_norm, _, _ = model(x)        # [B,7]
            q_pred = q_pred_norm * tgt_std + tgt_mean
            q_true = y * tgt_std + tgt_mean

            err = q_pred - q_true               # [B,7]
    ...
    mse_per_joint = sum_sq_err / n_samples      # radians^2 (if data in rad)
    rmse_per_joint = torch.sqrt(mse_per_joint)  # radians
    rmse_per_joint_deg = rmse_per_joint * 180.0 / math.pi
    mean_rmse_deg = rmse_per_joint_deg.mean().item()
    return mean_rmse_deg


def eval_ik_via_fk_pos_rmse_m(
    ik_model: torch.nn.Module,
    fk_model: torch.nn.Module,
    loader: DataLoader,
    device: torch.device,
    ik_input_mean,
    ik_input_std,
    ...
    """Evaluate IK model in task space by composing IK -> FK.

    For each validation sample (pose_des, q_true):

      pose_des (normalised) --(denorm IK stats)--> pose_real
      pose_real --(IK model)--> q_pred
      q_pred --(FK model)--> pose_pred

    We then compute RMSE of position error ||pos_pred - pos_des|| in metres.

    All *_mean/std arguments are expected to be numpy arrays of shape [1,7].
    """
    ik_model.eval()
    fk_model.eval()

    ik_in_mean = torch.from_numpy(ik_input_mean).float().to(device)
    ik_in_std = torch.from_numpy(ik_input_std).float().to(device)
    ik_tgt_mean = torch.from_numpy(ik_target_mean).float().to(device)
    ik_tgt_std = torch.from_numpy(ik_target_std).float().to(device)

    fk_in_mean = torch.from_numpy(fk_input_mean).float().to(device)
    fk_in_std = torch.from_numpy(fk_input_std).float().to(device)
    fk_tgt_mean = torch.from_numpy(fk_target_mean).float().to(device)
    fk_tgt_std = torch.from_numpy(fk_target_std).float().to(device)
    ...
    with torch.no_grad():
        for x_norm, y_norm in loader:
            ...
            # Denormalise desired pose using IK stats
            pose_des = x_norm * ik_in_std + ik_in_mean     # [B,7]

            # IK prediction in joint space (normalised -> real units)
            q_pred_norm, _, _ = ik_model(x_norm)
            q_pred = q_pred_norm * ik_tgt_std + ik_tgt_mean  # [B,7]

            # Normalise joints for FK network
            q_fk_norm = (q_pred - fk_in_mean) / fk_in_std
            pose_pred_norm = fk_model(q_fk_norm)
            pose_pred = pose_pred_norm * fk_tgt_std + fk_tgt_mean

            pos_des = pose_des[:, :3]
            pos_pred = pose_pred[:, :3]
            dist = torch.norm(pos_pred - pos_des, dim=1)
            sum_sq_dist += torch.sum(dist ** 2).item()
            n_samples += dist.size(0)
...
def main():
    ...
    fk_model = None
    fk_input_mean = fk_input_std = fk_target_mean = fk_target_std = None

    # Optional: load a trained FK model for IK evaluation in task space (meters)
    if args.mode == "ik" and getattr(args, "fk_checkpoint", None):
        fk_ckpt_path = args.fk_checkpoint
        if not os.path.exists(fk_ckpt_path):
            print(f"[WARN] FK checkpoint {fk_ckpt_path} not found; will skip IK->FK position eval.")
        else:
            print(f"[INFO] Loading FK checkpoint from {fk_ckpt_path} for IK evaluation.")
            try:
                fk_ckpt = torch.load(fk_ckpt_path, map_location=device, weights_only=False)
            except TypeError:
                fk_ckpt = torch.load(fk_ckpt_path, map_location=device)

            ckpt_mode = fk_ckpt.get("mode", "fk")
            if ckpt_mode != "fk":
                print(f"[WARN] FK checkpoint mode={ckpt_mode} (expected 'fk'); skipping IK->FK eval.")
            else:
                fk_hidden_dim = fk_ckpt.get("hidden_dim", args.hidden_dim)
                fk_num_blocks = fk_ckpt.get("num_blocks", args.num_blocks)

                fk_input_mean = fk_ckpt["input_mean"]
                fk_input_std = fk_ckpt["input_std"]
                fk_target_mean = fk_ckpt["target_mean"]
                fk_target_std = fk_ckpt["target_std"]

                fk_model = ResidualMLP(
                    input_dim,
                    output_dim,
                    hidden_dim=fk_hidden_dim,
                    num_blocks=fk_num_blocks,
                ).to(device)
                fk_model.load_state_dict(fk_ckpt["model_state_dict"])
                fk_model.eval()
    ...
    for epoch in range(args.epochs):
        ...
        # Extra metrics in physical units
        if args.mode == "fk":
            pos_rmse_m = eval_fk_pos_rmse_m(
                model,
                val_loader,
                device,
                full_dataset.target_mean,
                full_dataset.target_std,
            )
            writer.add_scalar("metric/val_pos_rmse_m", pos_rmse_m, epoch)
        elif args.mode == "ik":
            ...
            writer.add_scalar("metric/val_joint_rmse_deg", joint_rmse_deg, epoch)

            # If an FK checkpoint is provided, also evaluate IK->FK position error in metres.
            if fk_model is not None and fk_input_mean is not None:
                pos_rmse_m_via_fk = eval_ik_via_fk_pos_rmse_m(
                    model,
                    fk_model,
                    val_loader,
                    device,
                    full_dataset.input_mean,
                    full_dataset.input_std,
                    full_dataset.target_mean,
                    full_dataset.target_std,
                    fk_input_mean,
                    fk_input_std,
                ...
                writer.add_scalar("metric/val_pos_rmse_m_via_fk", pos_rmse_m_via_fk, epoch)
